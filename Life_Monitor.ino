...
void setup() {
  ...
  imu.getMres();
  int setup_state = 0;
  String name_to_welcome = "";
  while(bv_3 != 1){
    bv_1 = button_1.update();
    bv_2 = button_2.update();
    bv_3 = button_3.update();
    if (k==0) {
      Serial.println("Users");
      oled.clearBuffer();
      oled.setCursor(28,10);
      oled.print("Select User");
      oled.setCursor(2, 22);
      oled.print("dimitral");
      oled.setCursor(2, 35);
      oled.print("gdolphin");
      ...
      oled.setCursor(2, 60);
      oled.print("gmadonna");
      oled.drawFrame(0,12.5*(setup_state + 1),60,13);
      oled.sendBuffer();
      k=1;
    }
    if (bv_1==1) {
      setup_state = (setup_state + 1)%4;  // in order to scroll through the 4th menu, substates=11-14
      k=0;
    }
    if(bv_2 == 1){
...
//        k=1;
      ...
      if (bv_1==1) {
        ...
        if(k == 0){
          ...
          oled.setCursor(98, 61);
          oled.print("Back");
          if(millis() - last_blink_time > 2*BLINKING_INTERVAL){
            last_blink_time = millis();
          }
          else if(millis() - last_blink_time > BLINKING_INTERVAL){
            oled.drawFrame(95, 50, 32, 13);
          }
          else if(millis() - last_blink_time < BLINKING_INTERVAL){
  ...
  if(gps_working){
    while(GPSSerial.available()){
      if(gps.encode(GPSSerial.read())){
        ...
        if(gps.location.isValid() and update_timer > GPS_UPDATE_INTERVAL){
          Serial.println(using_gps_time);
         if(wifi.isConnected() == 0 and using_gps_time == 1){
    ...
    if(wifi.isConnected() && !wifi.isBusy() && update_timer > UPDATE_INTERVAL){
      ...
      post_gps_data = "&lat=" + String(new_lat,6) + "&lon=" + String(new_lon,6);
  ...
  if(hour() == 0 && minute() == 0 && alarm_true == 1){
    step_counter = 0;
    calories = 0;
    total_distance = 0;
    using_gps_time = 1;
...
void store_GPS_data() {
  ...
  distance = measure_distance(new_lat, new_lon, last_lat, last_lon);
  calc_speed();
  if(speed > .5 and distance < 0.152236){
