...
void loop() {
    if(message.indexOf("@")!= -1){
      ...
      if(keyword == "save"){
        ...
        String data_to_save = message.substring(username_end + 1, message.length());
        if(user == "dimitral"){
          if(SD.exists("dimitral.txt")){
            Serial.println("dimitral.txt exists");
          }
          SD.remove("dimitral.txt");
          myFile = SD.open("dimitral.txt", FILE_WRITE);
        }
        else if(user == "gdolphin"){
          if(SD.exists("gdolphin.txt")){
            Serial.println("gdolphin.txt exists");
          }
          SD.remove("gdolphin.txt");
          myFile = SD.open("gdolphin.txt", FILE_WRITE);
        }
        else if(user == "pkarnati"){
          if(SD.exists("pkarnati.txt")){
            Serial.println("pkarnati.txt exists");
          }
          SD.remove("pkarnati.txt");
          myFile = SD.open("pkarnati.txt", FILE_WRITE);
        }
        else if(user == "gmadonna"){
          if(SD.exists("gmadonna.txt")){
            Serial.println("gmadonna.txt exists");
          }
          SD.remove("gmadonna.txt");
          myFile = SD.open("gmadonna.txt", FILE_WRITE);
        }
        myFile.print(data_to_save);
        myFile.close();
        Serial.println("Data entered correctly");
      }
      if(keyword == "user"){
      ...
      if(keyword == "time"){
        ...
        sound_state = 1;
        speech = "";
        while(sound_state != 0 && Serial1.available() == 0){
          if(sound_state == 1){
            speech = the_hour + ".WAV";
